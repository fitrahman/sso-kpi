        ...
        <div class="p-6 max-w-7xl w-full mx-auto space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <form method="GET" action="{{ route('admin.clients.users', $client->id) }}" class="flex items-center gap-2">
                        <div class="relative">
                            <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama / email..."
                                class="w-64 border border-slate-300 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-kpi-500">
                        ...
                        @if (!empty($search))
                            <a href="{{ route('admin.clients.users', $client->id) }}" class="px-3 py-2 border border-slate-300 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50">Reset</a>
                        @endif
                    </form>
                </div>
