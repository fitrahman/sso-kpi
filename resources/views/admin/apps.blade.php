...
<body class="bg-slate-50 text-slate-800 h-screen flex overflow-hidden">
                ...
                @foreach ($clients as $client)
                    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col {{ $client->is_maintenance ? 'border-amber-200' : '' }} {{ !$client->is_visible ? 'opacity-60' : '' }}">
                        ...
                        <div class="p-4 flex items-center gap-2 flex-wrap mt-auto">
                            <!-- Edit Button -->
                            <button onclick="openEditModal({{ $client->id }}, '{{ addslashes($client->name) }}', '{{ addslashes($client->description ?? '') }}', '{{ addslashes($client->maintenance_message ?? '') }}', {{ $client->display_order }}, {{ $client->is_visible ? 1 : 0 }}, '{{ $client->logo_path ? Storage::url($client->logo_path) : '' }}')"
        ...
        <div class="modal-content bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10">
            ...
            <form id="editForm" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    {{-- Current image preview (shown only if image exists) --}}
                    <div id="currentLogoPreview" class="hidden mb-3 p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center gap-3">
                        <img id="currentLogoImg" src="" alt="Gambar saat ini" class="h-14 w-24 object-cover rounded-lg border border-slate-200 shadow-sm">
                        <div class="flex-1">
                            <p class="text-xs text-slate-500 mb-1.5">Gambar saat ini</p>
                            <form id="deleteLogoForm" method="POST" onsubmit="return confirm('Hapus gambar aplikasi ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-200 text-red-700 rounded-lg text-xs font-semibold hover:bg-red-100 transition-colors">
                    ...
                    <p class="text-xs text-slate-400 mt-1">Format: PNG, JPG, SVG. Maks. 2MB. Unggah file baru untuk mengganti gambar.</p>
                </div>
    ...
    <script>
        function openEditModal(id, name, description, maintenanceMsg, displayOrder, isVisible, logoUrl) {
            ...
            // Handle current logo preview
            const preview = document.getElementById('currentLogoPreview');
            const img     = document.getElementById('currentLogoImg');
            const delForm = document.getElementById('deleteLogoForm');
            if (logoUrl) {
                img.src = logoUrl;
                delForm.action = '/admin/applications/' + id + '/logo';
